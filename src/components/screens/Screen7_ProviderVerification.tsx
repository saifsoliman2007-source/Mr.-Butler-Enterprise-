...
import { Mail, CheckCircle2, RefreshCw, ArrowLeft, Edit3, AlertCircle, Building2 } from 'lucide-react';
...
export const Screen7_ProviderVerification: React.FC<Screen7Props> = ({
  email,
  businessName,
  onNavigate,
  lang,
  ...
  const handleVerify = () => {
    if (otpValue.length < 6) {
      setErrorMsg('Please enter all 6 digits of your business verification code.');
      return;
    }
    onNavigate(8); // Provider Reg Complete
  };
    ...
    <div className="min-h-full w-full bg-[#F8FAFC] dark:bg-slate-950 text-[#1E293B] dark:text-slate-100 flex flex-col justify-between p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="max-w-md w-full mx-auto space-y-6 my-auto text-center">
        ...
        <div className="space-y-3 flex flex-col items-center">
          ...
          <div className="p-3.5 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 text-xs text-[#64748B] dark:text-slate-300 space-y-1">
            ...
            <div className="flex items-center justify-center gap-2 font-mono font-bold text-[#1D4ED8] dark:text-blue-400 text-sm">
              <span>{email || 'provider@business.com'}</span>
              <button
                onClick={() => onNavigate(6)}
                title={t.changeEmailAddress}
                className="p-1 rounded hover:bg-[#F1F5F9] dark:hover:bg-slate-800 text-[#64748B] transition"
              >
