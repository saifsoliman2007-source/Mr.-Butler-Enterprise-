            ...
            onClick={() => {
              const selectedItemsList = items
                ...
                .map(it => ({
                  ...
                  notes: `Treatment: ${it.serviceType}`
                }));

              if (onBookingSubmit) {
                onBookingSubmit({
                  category: 'Shoe Fix & Repair',
                  serviceTitle: 'Master Cobbler Footwear Restoration & Care',
                  items: selectedItemsList.length > 0 ? selectedItemsList : [
                    { id: 'it-default', name: 'Master Cobbler Footwear Service', quantity: totalItems || 1, price: calculatedTotal || 65 }
                  ],
                  estimatedPrice: calculatedTotal || 65,
                  requestedDateTime: `${dropoffDate} (${dropoffTime})`,
                  ...
                  uploadedImages: shoePhoto ? [shoePhoto] : [],
                });
              }
              setIsBooked(true);
            }}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full p-5 text-center space-y-4 shadow-2xl border border-[#D9E3F6] dark:border-slate-800 animate-fadeIn">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                  {isRTL 
                    ? `سيقوم مندوب الفاليه باستلام الأحذية في ${dropoffDate} خلال الفترة ${dropoffTime}. تم إرسال صور المعاينة بنجاح لورشة الإصلاح.` 
                    : `Our valet will arrive to collect your footwear on ${dropoffDate} during ${dropoffTime}. Your footwear condition photos have been routed to the master cobbler.`}
                </p>
              </div>
              ...
              {shoePhoto && (
                <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center gap-2.5 text-left rtl:text-right">
                  <img
                    src={shoePhoto}
                    alt="Footwear Intake Photo"
                    className="w-10 h-10 rounded-lg object-cover border border-slate-300 dark:border-slate-600"
                  />
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] font-bold text-emerald-600 block">
                      {isRTL ? 'تم ربط صورة الحذاء بالطلب' : 'Condition Photo Linked'}
                    </span>
                    <span className="text-[10px] text-slate-500 truncate block">
                      {specialInstructions || (isRTL ? 'جاهز لمعاينة الإسكافي' : 'Ready for cobbler inspection')}
                    </span>
                  </div>
                </div>
              )}
