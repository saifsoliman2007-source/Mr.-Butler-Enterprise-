import React from 'react';
import { Language, Role, ScreenId, FeatureToggles } from '../../types';
import { translations } from '../../data/translations';
import { EGEC } from '../EGEC';
import { Mail, ArrowLeft, ShieldAlert } from 'lucide-react';

interface Screen2Props {
  role: Role;
  toggles: FeatureToggles;
  onNavigate: (screen: ScreenId) => void;
  lang: Language;
}

export const Screen2_RegMethod: React.FC<Screen2Props> = ({
  role,
  toggles,
  onNavigate,
  lang,
}) => {
  const t = translations[lang] || translations.en;

  const handleSocialClick = (provider: string) => {
    // Quick social access bypass to completion screen or email prompt
    onNavigate(role === 'consumer' ? 5 : 8);
  };

  const hasQuickAccess = toggles.googleAuth || toggles.facebookAuth || toggles.appleAuth;

  return (
    <div className="min-h-full w-full bg-[#F8FAFC] dark:bg-slate-950 text-[#1E293B] dark:text-slate-100 flex flex-col justify-between p-4 sm:p-6 lg:p-8 space-y-6">
      
      {/* Top Header */}
      <div className="flex items-center justify-between">
        <button
          onClick={() => onNavigate(1)}
          className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 text-[#475569] dark:text-slate-300 hover:bg-[#F1F5F9] transition flex items-center gap-1.5 text-xs font-semibold"
        >
          <ArrowLeft className="w-4 h-4 text-[#3B82F6]" />
          <span>Back</span>
        </button>

        <span className="text-xs font-semibold px-3 py-1 rounded-full bg-[#EFF6FF] text-[#1D4ED8] dark:bg-blue-950 dark:text-blue-300 border border-[#E2E8F0] dark:border-blue-800 capitalize">
          Role: {role === 'consumer' ? t.roleConsumer : t.roleProvider}
        </span>
      </div>

      {/* Main Container */}
      <div className="max-w-md w-full mx-auto space-y-6 my-auto">
        
        {/* Title */}
        <div className="text-center space-y-2 flex flex-col items-center">
          <EGEC size="md" shape="rounded-xl" withAura={true} withSheen={true} withBeacon={true} beaconStatus="online" />
          <h1 className="text-2xl font-bold text-[#0F172A] dark:text-white tracking-tight">
            {role === 'consumer' ? 'Consumer Registration' : 'Service Provider Access'}
          </h1>
          <p className="text-xs text-[#64748B] dark:text-slate-400">
            Select your preferred authentication method to continue
          </p>
        </div>

        {/* Quick Access Section (Subject to Administrator Feature Toggles) */}
        {hasQuickAccess && (
          <div className="space-y-3">
            <p className="text-xs font-bold text-[#64748B] uppercase tracking-wider text-center">
              {t.quickAccess}
            </p>

            <div className="space-y-2.5">
              {toggles.googleAuth && (
                <button
                  onClick={() => handleSocialClick('Google')}
                  className="w-full py-3 px-4 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 text-[#0F172A] dark:text-slate-100 font-semibold text-xs shadow-xs hover:border-[#3B82F6] transition flex items-center justify-center gap-3 cursor-pointer min-h-[44px]"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
                  </svg>
                  <span>{t.continueWithGoogle}</span>
                </button>
              )}

              {toggles.facebookAuth && (
                <button
                  onClick={() => handleSocialClick('Facebook')}
                  className="w-full py-3 px-4 rounded-xl bg-[#1877F2] text-white font-semibold text-xs shadow-xs hover:bg-[#166fe5] transition flex items-center justify-center gap-3 cursor-pointer min-h-[44px]"
                >
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                  <span>{t.continueWithFacebook}</span>
                </button>
              )}

              {toggles.appleAuth && (
                <button
                  onClick={() => handleSocialClick('Apple')}
                  className="w-full py-3 px-4 rounded-xl bg-[#0F172A] text-white font-semibold text-xs shadow-xs hover:bg-[#1E293B] transition flex items-center justify-center gap-3 cursor-pointer min-h-[44px]"
                >
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 6.09c.66-.8 1.11-1.92.99-3.04-.96.04-2.12.64-2.8 1.44-.61.71-1.14 1.86-1 2.97 1.08.08 2.16-.57 2.81-1.37z" />
                  </svg>
                  <span>{t.continueWithApple}</span>
                </button>
              )}
            </div>
          </div>
        )}

        {!hasQuickAccess && (
          <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs flex items-center gap-2">
            <ShieldAlert className="w-4 h-4 shrink-0" />
            <span>Social Quick Access methods currently disabled by administrator policy.</span>
          </div>
        )}

        {/* Divider */}
        {hasQuickAccess && toggles.emailReg && (
          <div className="relative flex items-center justify-center my-4">
            <div className="w-full border-t border-[#E2E8F0] dark:border-slate-800" />
            <span className="absolute bg-[#F8FAFC] dark:bg-slate-950 px-3 text-xs text-[#64748B] font-medium">
              {t.orDivider}
            </span>
          </div>
        )}

        {/* Standard Email Registration */}
        {toggles.emailReg && (
          <div className="space-y-3">
            <p className="text-xs font-bold text-[#64748B] uppercase tracking-wider text-center">
              {t.standardRegistration}
            </p>

            <button
              onClick={() => onNavigate(role === 'consumer' ? 3 : 6)}
              className="w-full py-3.5 px-4 rounded-xl bg-[#0F172A] hover:bg-[#1E293B] text-white font-bold text-xs shadow-md transition flex items-center justify-center gap-2.5 cursor-pointer min-h-[44px]"
            >
              <Mail className="w-4 h-4 text-[#3B82F6]" />
              <span>{t.registerByEmail}</span>
            </button>
          </div>
        )}

        {/* Bottom Sign in prompt */}
        <div className="text-center pt-4">
          <p className="text-xs text-[#64748B] dark:text-slate-400">
            {t.alreadyHaveAccount}{' '}
            <button
              onClick={() => onNavigate(9)}
              className="font-bold text-[#1D4ED8] dark:text-blue-400 hover:underline"
            >
              {t.signIn}
            </button>
          </p>
        </div>

      </div>

      <div />
    </div>
  );
};
